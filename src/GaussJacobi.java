...
public class GaussJacobi extends JFrame {

    // Gauss-Jacobi theme: plum/purple — matches MainUI button Color(221, 160, 221)
    private static final Color THEME      = new Color(171,  71, 188);  // vivid purple
    private static final Color THEME_DARK = new Color(106,  27, 154);  // deep purple for border/title
    private static final Color THEME_TINT = new Color(243, 229, 245);  // light lavender for alternating rows

    private JSpinner sizeSpinner;
    private JPanel matrixPanel;
    private JTextField[][] matrixFields;   // [row][col]: cols 0..n-1 = coefficients, col n = constant
    ...
    private int n = 3; // default size

    public GaussJacobi() {
        setTitle("Gauss-Jacobi Method");
        setSize(950, 680);
        ...
        // ===== HEADER =====
        JLabel header = new JLabel("GAUSS-JACOBI METHOD", SwingConstants.LEFT);
        ...
        // Columns built dynamically — start empty
        ...
        // Build default 3x3 matrix on open
        buildMatrix();
    }
    ...
    private void buildMatrix() {
        ...
        for (int j = 0; j <= n; j++) {
            gbc.gridx = j;
            JLabel lbl;
            if (j < n) lbl = new JLabel("x" + (j + 1), SwingConstants.CENTER);
            else        lbl = new JLabel("b",            SwingConstants.CENTER);
        ...
        for (int i = 0; i < n; i++) {
            ...
            // Actually we use a separate column for it — shift fields right
            // We'll use col 0 for label, cols 1..n for coefficients, col n+1 for b
            // But simpler: just label each row on the left outside grid. Let's embed it.
    ...
    private void compute() {
        ...
        try {
            ...
            for (int iter = 1; iter <= 100; iter++) {
                // Jacobi update: use OLD x for all equations
                for (int i = 0; i < n; i++) {
                    double sum = b[i];
                    for (int j = 0; j < n; j++) {
                        if (j != i) sum -= A[i][j] * x[j];
                    }
                    xNew[i] = round2(sum / A[i][i]);
                }
                ...
                for (int i = 0; i < n; i++) {
                    if (iter == 1) {
                        ...
                        Ea[i] = round2(Math.abs(xNew[i] - x[i]));
                ...
                row[0] = iter;
                for (int i = 0; i < n; i++) row[1 + i]     = String.format("%.2f", xNew[i]);
